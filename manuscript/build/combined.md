...
**Stage 2: Keyword matching.** We match lemmatized keywords from each ideological category definition against the lemmatized speech text. This provides an inspectable baseline: every match can be traced to a specific dictionary entry. It also serves as an interpretability anchor for the meta-classifier: the meta-learner can learn to discount or amplify keyword signals, but the raw counts remain available for audit.
