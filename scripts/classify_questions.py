#!/usr/bin/env python3
"""Classify written questions (skriftliga frågor) and persist results to Parquet.

Reads from data/parquet/questions.parquet, classifies each row, and appends
classification rows to data/parquet/classifications.parquet.

Usage:
    uv run python scripts/classify_questions.py
    uv run python scripts/classify_questions.py --limit 100
"""
...
def classify_questions_parquet(
    questions_parquet: str | Path = "data/parquet/questions.parquet",
    classifications_out: str | Path = "data/parquet/classifications.parquet",
    limit: Optional[int] = None,
) -> int:
    defs = load_definitions()
    qp = Path(questions_parquet)
    out_p = Path(classifications_out)

    if not qp.exists():
        print(f"No questions parquet found at {qp}")
        return 0

    questions = pd.read_parquet(qp)

    # Determine already classified questions
    ...
    to_classify = questions[~questions["id"].astype(str).isin(classified_ids)].copy()
    if limit:
        to_classify = to_classify.head(limit)

    print(f"Classifying {len(to_classify)} of {len(questions)} questions ...", file=sys.stderr)
    ...
    for _, r in to_classify.iterrows():
        ...
        text = (r.get("title") or "") + "\n" + (r.get("text") or "")
        try:
            results = classify_motion(motion_id=qid, text=text, categories=defs)
        except Exception as e:
            print(f"Failed to classify {qid}: {e}", file=sys.stderr)
            continue
...
def main():
    parser = argparse.ArgumentParser(description="Classify written questions and persist to Parquet")
    ...
    classified = classify_questions_parquet(
...
if False:
    # Graphify hint: questions classification uses the same classify_motion() pipeline as motions
    from swedish_parliament_policy_classifier.exports import load_definitions, classify_motion