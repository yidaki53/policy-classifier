...
def _load_corpus_stats(repo_root: Path) -> dict[str, Any]:
    ...
    if motions_path.exists():
        mdf = pd.read_parquet(motions_path, columns=["id", "metadata", "date"])
        stats["motions_n"] = int(mdf["id"].nunique())
        years: list[int] = []
        # First try direct date column (most reliable)
        if "date" in mdf.columns:
            years.extend(_to_year_series(mdf["date"]).tolist())
        # Fallback to rm field in metadata
        if not years:
...
def _build_context(repo_root: Path, manuscript_dir: Path, analysis_dir: Path, journal_profile: Path, bib_path: Path) -> dict:
    ...
    corpus_counts_paragraph = (
        f"- **Motions**: `n={_fmt_int(corpus.get('motions_n'))}` documents spanning {_fmt_int(corpus.get('motions_year_min'))}-{_fmt_int(corpus.get('motions_year_max'))}, retrieved via the Riksdag Open Data API (`/api/v1/dokument/sok?typ=mot`). Each motion includes metadata (party, date, title, signatories) and full text. Documents are deduplicated by `dok_id` and filtered to exclude committee reports and government propositions unless explicitly analyzed as comparison material.\n"
        f"- **Speeches**: `n={_fmt_int(corpus.get('speeches_n'))}` unique plenary speeches spanning {_fmt_int(corpus.get('speeches_year_min'))}-{_fmt_int(corpus.get('speeches_year_max'))}, retrieved via the Riksdag anförande API (`/api/v1/anforande/sok`). Each speech includes speaker, party affiliation, and timestamp. Speeches shorter than 50 characters after HTML stripping are excluded.\n"
        f"- **Votes**: `n={_fmt_int(corpus.get('vote_events_n'))}` unique roll-call vote events spanning {_fmt_int(corpus.get('votes_year_min'))}-{_fmt_int(corpus.get('votes_year_max'))}, retrieved via the Riksdag votering API (`/api/v1/votering/sok`). Each vote event includes party-level voting records (yes/no/abstain/absent) and metadata linking to the underlying proposition."
    )
