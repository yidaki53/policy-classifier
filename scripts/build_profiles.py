...
def compute_vote_profiles(parquet_dir: str, votering_dir: str, lam: float, reference_date: pd.Timestamp):
    ...
    if "date" in nm.columns:
        nm["date_parsed"] = pd.to_datetime(nm["date"], errors="coerce")
    else:
    ...
    for p in sorted(glob.glob(pattern)):
        ...
        for _, r in df.iterrows():
            ...
            party = r["parti"]
            vot_date = pd.to_datetime(r.get("datum", None), errors="coerce")
...
def compute_speech_profiles(speech_classifications_path: str, speech_parquet_dir: str, lam: float, reference_date: pd.Timestamp):
    ...
    merged["date_parsed"] = pd.to_datetime(merged.get("date", None), errors="coerce")
...
def main():
    ...
    motion_dates = pd.to_datetime(nm["date"], errors="coerce") if not nm.empty else pd.Series([], dtype="datetime64[ns]")

    # speech latest date
    speech_meta = load_speech_metadata(args.speech_parquet_dir)
    speech_dates = speech_meta["date"] if not speech_meta.empty else pd.Series([], dtype="datetime64[ns]")

    all_dates = pd.concat([motion_dates.dropna(), speech_dates.dropna()])
    reference_date = all_dates.max() if not all_dates.empty else pd.Timestamp.now()
