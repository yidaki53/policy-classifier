...
- For semantic extraction, prefer the local Ollama model `qwen-32k` when available. Confirm the model is running with `ollama list` (the model name is `qwen-32k`) and ensure the Ollama daemon is active. Scripts that support Ollama can be enabled with the `--ollama` flag (for example, `scripts/classify_speeches_parquet.py --ollama`); using the local Ollama avoids downloading large remote models. If `qwen-32k` is not present, fall back to `graphify update` (AST-only) and note the skip.
