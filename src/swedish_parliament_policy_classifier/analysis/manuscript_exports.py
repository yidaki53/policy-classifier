...
def _filter_overlay_profiles(profiles: pd.DataFrame) -> pd.DataFrame:
...
def plot_modality_overlay_figure(
    ...
    fig, axes = plt.subplots(n_rows, n_cols, figsize=(5.8 * n_cols, 3.8 * n_rows), squeeze=False, sharey=True)
    modalities = ["speech", "motion", "vote"]
    colors = {"speech": "#1f77b4", "motion": "#ff7f0e", "vote": "#2ca02c"}

    y_max = float(np.nanmax(profiles["proportion"].to_numpy(dtype=float))) if len(profiles) else 1.0
    y_max = min(1.0, max(0.35, y_max * 1.15))
    ...
    for i, party in enumerate(parties):
        ...
        for m in modalities:
            ...
            ax.plot(x, s, marker="o", linewidth=2.1, markersize=4.8, label=m, color=colors[m], alpha=0.95)

        ax.set_title(party, fontsize=11)
        ax.set_xticks(x)
        ax.set_xticklabels(IDEOLOGY_ORDER, rotation=35, ha="right", fontsize=9)
    ...
    fig.suptitle("Party ideology profiles by modality (speech vs motion vs vote)", y=0.99, fontsize=13)
    fig.legend(handles, labels, loc="upper center", bbox_to_anchor=(0.5, 0.962), ncol=3, frameon=False)
    fig.tight_layout(rect=[0.01, 0.01, 1.0, 0.90])
